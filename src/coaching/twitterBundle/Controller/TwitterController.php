...
namespace coaching\twitterBundle\Controller;

use coaching\twitterBundle\Entity\Tweet;
...
class TwitterController extends Controller
{
    ...
    {
        return $this->render('coachingBundle:coaching:index.html.twig', array('name' => "twitter"));
    }
    public function addAction()
    {
		$doctrine = $this->getDoctrine();
    	$manager = $doctrine->getManager();
    	$html = file_get_html("https://twitter.com/search?q=sport%20lang%3Afr&src=typd&f=realtime");
		$resultats = $html->find("ol#stream-items-id", 0);
		$list = $resultats->find(".tweet");
		$tweetid = "data-tweet-id";
		$i = 0;
		while (isset($list[$i]))
		{
			$tweet = $list[$i]->find("div.content", 0);
			$id = $list[$i]->$tweetid;
			$avatar = $tweet->find("img.avatar", 0)->src;
			$fullname = $tweet->find("strong.fullname", 0)->plaintext;
			$username = $tweet->find("span.username", 0)->plaintext;
			$time = $tweet->find("small.time", 0);
			$content = $tweet->find("p.tweet-text", 0)->plaintext;
			$time = new \DateTime("now");
			$tweetdb = new Tweet($id, $avatar, $fullname, $username, $time, $content);
	    	$manager->persist($tweetdb);
			$i++;
		}
		$manager->flush();
        return $this->render('coachingBundle:coaching:index.html.twig', array('name' => "twitter"));
    ...
    private function getTweet()
    {
		...
		while (isset($list[$i]))
		{
			$tweet = $list[$i]->find("div.content", 0);
			$id = $list[$i]->$tweetid;
			$avatar = $tweet->find("img.avatar", 0);
			$fullname = $tweet->find("strong.fullname", 0)->plaintext;
			$username = $tweet->find("span.username", 0)->plaintext;
			$time = $tweet->find("small.time", 0);
			$content = $tweet->find("p.tweet-text", 0);
			echo $id.'>'.$avatar.' '.$fullname.' '.$username.' '.$time.' '.$content.'<br />';
			$i++;
		}
    }
}
